...
items.get("/", async (c) => {
  ...
  const rows = showWatched
    ? await query.orderBy(desc(schema.watchItems.watchedAt), desc(schema.watchItems.id))
    : await query.orderBy(asc(schema.watchItems.position));
...
items.patch("/:id", async (c) => {
  ...
  const updateSet: Partial<typeof schema.watchItems.$inferInsert> = {
    ...body,
    updatedAt: new Date(),
  };
  if (body.watched === true) {
    updateSet.watchedAt = new Date();
