...
function renderSoundtrackLink(
  ...
  if (mediaType === "movie" && imdbId) {
    return html`<a href="${`https://www.imdb.com/title/${imdbId}/soundtrack/`}" class="btn-external" target="_blank" rel="noopener noreferrer">
      IMDb${raw("&nbsp;↗")}
    </a>`;
  }
  const q = year ? `${title} ${year}` : title;
  return html`<a href="${`https://www.tunefind.com/search?query=${encodeURIComponent(q)}`}" class="btn-external" target="_blank" rel="noopener noreferrer">
    Tunefind${raw("&nbsp;↗")}
  </a>`;
}

export function renderDetailPage(data: DetailPageData): Html {
...
function renderCTA(data: DetailPageData, qs: string): Html {
